...
const teachervalue = JSON.parse(localStorage.getItem('education'))
...
// axios.get(`${BaseUrl}/view-timetable-subjects/${classes}`).then
// (res=> {
//   console.log('ss',res)
...
useEffect(()=>{
    ...
    axios.get(`${BaseUrl}/viewteacherlist/${teachervalue.data.school_id}`).then
    (res=> {
        console.log('er',res)
...
axios.get(`${BaseUrl}/view-teacher-subjects/${teacher}`).then(
    res=> {
        console.log('qc',res.data.data)
        setGetdata(res.data.data)
    }    
)
   }
    ...
    <>
      ...
      <Row form>
        <Col md={6}>
        <FormGroup>
        <Label for="exampleSelect">Teachers Name</Label>
        <Input onChange={(e)=> setTeacher(e.target.value)} required value={teacher} type="select" name="select" id="exampleSelect">
        ...
        <Col md={4}>
        <Button onClick={getTeacherdata} style={{transform:'translateY(33px)'}}>Search</Button>
        </Col>
      </Row>
