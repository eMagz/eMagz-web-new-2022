...
const Index = () => {
  ...
  const submitForms = () => {
    ...
    axios.post(`${BaseUrl}/addschool`, body).then(
      res => {
        axios.post(`${BaseUrl}/uploadschoolbanner/${res.data.data._id}`, bannerPhoto).then(
          restwo => {
            console.log('BANNER==========', restwo)
            if (res.data.status && restwo.data.status === true) {
              swal(res.data.msg)
            } else {
              swal(res.data.msg)
            }
        ...
        axios.post(`${BaseUrl}/uploadschoollogo/${res.data.data._id}`, logoimage).then(
          restwo => {
          ...
          <div className='approval_form'  >
            ...
            <div className='applybuttons'>
              {resdata.status && resonedata.status === true ? <div className={classes.root}>
                ...
                <button onClick={() => submitForms()} className='applybtn applybtn1'>APPLY</button>
              }
            </div>
